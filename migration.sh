...
# Pantheon Settings
pantheon_env='dev'
label='A test of my script'

# Instance to export
instance='5cd5e902e1fa27ae427accc5'
# Get instance path
site_name=$(curl -s -L http://osr-dev-util01.int.colorado.edu/atlas/sites/$instance | jq -r '.path')

# Request site backups
status=$(curl -s --netrc-file credentials.netrc -H "Content-Type: application/json" -X POST -o /dev/null -I -w '%{http_code}' https://osr-{$env}-util01.int.colorado.edu/atlas/sites/{$instance}/backup)

# Conditional to check for backup status
if [ $status -eq 200 ]; then
	echo "$instance backup Initiated. Waiting $backup_wait seconds for backup to finish..."
else
...
backups_json=$(curl -s -g 'https://osr-'${env}'-util01.int.colorado.edu/atlas/backup?sort=-_updated&max_results=1&where={"site":"'${instance}'"}')
...
then
	...
	sleep $backup_wait
	backups_json_retry=$(curl -s -g 'https://osr-'${env}'-util01.int.colorado.edu/atlas/backup?sort=-_updated&max_results=1&where={"site":"'${instance}'"}')
...
# echo "Exporting..."
scp -i ~/.ssh/id_rsa osr-${env}-util01.int.colorado.edu:/nfs/dev_backups/backups/$site_database ./database

echo "Your database backup is ${site_database}"
scp -i ~/.ssh/id_rsa osr-${env}-util01.int.colorado.edu:/nfs/dev_backups/backups/$site_files ./files
...
mysqldump -u$mysql_username -p$mysql_password DATABASENAME | gzip > $site_database

echo "Database upload complete"

echo "Rsync'ing files to pantheon"
tar -xzvf ./files/$site_files
rm ./files/$site_files
terminus rsync ./files $site_name.dev:files
